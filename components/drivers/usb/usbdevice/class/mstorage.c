...
static rt_size_t _mode_sense_6(ufunction_t func, ustorage_cbw_t cbw)
{
    ...
    buf[0] = 3;
    buf[1] = 0;
    buf[2] = data->ro ? 0x80:0x00; /* WP, DPOFUA */
    buf[3] = 0;
...
static rt_size_t _read_10(ufunction_t func, ustorage_cbw_t cbw)
{
    ...
    rt_device_read(data->disk, data->block, data->ep_in->buffer, 1);
...
static rt_size_t _write_10(ufunction_t func, ustorage_cbw_t cbw)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_USB, ("_write_10 count 0x%x 0x%x\n",
                                data->count, data->geometry.sector_count));

    data->csw_response.data_reside = data->cb_data_size;
    
    data->ep_out->request.buffer = data->ep_out->buffer;
    data->ep_out->request.size = data->geometry.bytes_per_sector;    
    data->ep_out->request.req_type = UIO_REQUEST_READ;    
...
static rt_err_t _ep_in_handler(ufunction_t func, rt_size_t size)
{
    ...
    case STAT_CSW:
        if(size != SIZEOF_CSW)
        {
            rt_kprintf("Size of csw command error, %d\n", size);
            dcd_ep_stall(func->device->dcd, data->ep_in);            
        }
        else
        {
            RT_DEBUG_LOG(RT_DEBUG_USB, ("return to cbw status\n"));
            dcd_ep_read(func->device->dcd, data->ep_out, 
                data->ep_out->buffer, SIZEOF_CBW);
     ...
     case STAT_CMD:
        ...
        if(data->csw_response.data_reside)
        {
            ...
            {
                dcd_ep_stall(func->device->dcd, data->ep_out);
            }
            else
            {
                dcd_ep_stall(func->device->dcd, data->ep_in);
     ...
     case STAT_SEND:        
        ...
        {
            rt_device_read(data->disk, data->block, data->ep_in->buffer, 1);
            dcd_ep_write(func->device->dcd, data->ep_in, data->ep_in->buffer,
...
static rt_err_t _interface_handler(ufunction_t func, ureq_t setup)
{
    ...
    case USBREQ_GET_MAX_LUN:
        ...
        {
            dcd_ep_stall(func->device->dcd, 0);
        }
        else
        {
            dcd_ep_write(func->device->dcd, 0, &lun, 1);
        }
        break;
    case USBREQ_MASS_STORAGE_RESET:
        ...
        {
            dcd_ep_write(func->device->dcd, 0, RT_NULL, 0);
        }
        break;
    default:
...
static rt_err_t _function_run(ufunction_t func)
{
    ...
    data->buffer = (rt_uint8_t*)rt_malloc(data->geometry.bytes_per_sector);
    if(data->buffer == RT_NULL)
        return -RT_ENOMEM;
    data->ep_out->buffer = data->buffer;
    data->ep_in->buffer = data->buffer;

    dcd_ep_read(func->device->dcd, data->ep_out, data->ep_out->buffer, SIZEOF_CBW);
...
#ifdef RT_USB_MSTORAGE_READONLY
    data->ro = 1;
#endif
