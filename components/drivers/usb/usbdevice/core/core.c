...
static rt_err_t _get_qualifier_descriptor(struct udevice* device, ureq_t setup)
{
    if(device->dev_qualifier)
    {
        ...
        dcd_ep_write(device->dcd, 0, (rt_uint8_t*)device->dev_qualifier,
...
static rt_err_t _set_config(struct udevice* device, ureq_t setup)
{
    ...
    for (i=cfg->func_list.next; i!=&cfg->func_list; i=i->next)
    {
        ...
        /* after running all endpoints, then run function */
        if(func->ops->run != RT_NULL)
            func->ops->run(func);
    }
...
static rt_err_t _set_address(struct udevice* device, ureq_t setup)
{
    ...
    if(ret != RT_EOK)
    {
        rt_kprintf("_set_address timeout\n");
    }
...
static rt_err_t _standard_request(struct udevice* device, ureq_t setup)
{
    ...
    case USB_REQ_TYPE_ENDPOINT:
        ...
        case USB_REQ_CLEAR_FEATURE:
        {
            ...
            ep = rt_usbd_find_endpoint(device, RT_NULL, setup->index);
            if(USB_EP_HALT == setup->value && ep->is_stall == RT_TRUE)
            {
                dcd_clear_feature(dcd, setup->value, setup->index);
                dcd_send_status(dcd);
                ep->is_stall = RT_FALSE;  
...
/**
 * This function will stop all function.
 *
 * @param device the usb device object.
 *
 * @return RT_EOK.
...
/**
 * This function will run all function.
 *
 * @param device the usb device object.
 *
 * @return RT_EOK.
...
/**
 * This function will reset all function.
 *
 * @param device the usb device object.
 *
 * @return RT_EOK.
...
rt_size_t rt_usbd_io_request(udevice_t device, uep_t ep, uio_request_t req)
{
    ...
    if(ep->is_stall == RT_FALSE)
    {
        switch(req->req_type)
        {
        case UIO_REQUEST_READ:
            size = dcd_ep_read(device->dcd, ep, req->buffer, req->size);
            break;
        case UIO_REQUEST_WRITE:
            size = dcd_ep_write(device->dcd, ep, req->buffer, req->size);            
            break;
        default:
...
static void rt_usbd_thread_entry(void* parameter)
{
    while(1)
    {
        ...
        case USB_MSG_DATA_NOTIFY:
            ...
            if (device->state != USB_STATE_CONFIGURED)
                break;
            ep = rt_usbd_find_endpoint(device, &func, msg.content.ep_msg.ep_addr);
            if(ep != RT_NULL)
                ep->handler(func, msg.content.ep_msg.size);
            else
